...
$query=mysqli_query($conn,"SELECT * FROM (SELECT Employees_ID , Job_Department ,Position FROM tbl_employees_information) tbl_employees_information
						...
						JOIN (SELECT Employees_ID, COUNT(Date_Attendance) as Number_Attendance FROM tbl_time_entries  WHERE Date_Attendance BETWEEN '$from' AND '$to' GROUP by Employees_ID ) tbl_time_entries 
...
$pdf->cell(90,6,'SSS: '.$data['SSS'],1,1,'L');
$pdf->cell(90,6,'OT: '. $data['OT_Salary'],1,0,'L');
$pdf->cell(90,6,'PHILHEALTH '.$data['PHILHEALTH'],1,1,'L');
$pdf->cell(90,6,'',1,0,'L');
$pdf->cell(90,6,'PAG-IBIG: ' .$data['PAGIBIG'],1,1,'L');
$pdf->cell(90,6,'GROSS-SALARY: '. $data['Sum_Daily_Salary'],1,0,'L');
$pdf->cell(90,6,'TOTAL-DEDUCTION: ' . "1500" ,1,1,'L');
$pdf->cell(180,6,'NET-SALARY: ' . $data['Sum_Daily_Salary'] - $data['Total_Deduction'],1,0,'C');

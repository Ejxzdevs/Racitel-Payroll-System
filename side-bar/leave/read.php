...
// record
	...
	tbl_personal_information.Employees_ID "; 
...
// leave record
	...
	JOIN (SELECT Employees_ID, First_Name, Last_Name from tbl_personal_information where Employees_ID = '$id') tbl_personal_information ON tbl_employees_information.Employees_ID = tbl_personal_information.Employees_ID 
		LEFT JOIN (SELECT Employees_ID , Leave_Types , Leave_Date , Leave_Status  from tbl_file_leave Where Leave_Date Between '$first_date' AND '$Last_date' AND Employees_ID = '$id') tbl_file_leave ON tbl_personal_information.Employees_ID = tbl_file_leave.Employees_ID
		LEFT JOIN (SELECT Employees_ID, Limit_Leave from tbl_leave_limit where Employees_ID = '$id') tbl_leave_limit ON tbl_file_leave.Employees_ID = tbl_leave_limit.Employees_ID WHERE tbl_file_leave.Leave_Status = 'Accepted' ";
...
// filter leave record
	...
	$Last_date = "{$year}-12-01";
 	...
 	$sql1 = "SELECT * FROM(SELECT Employees_ID,Employee_Image ,@dept_id:= Department_ID AS Department_ID,(SELECT `Department_Name` FROM tbl_department WHERE `Department_ID` = @dept_id ) as Job_Department, @pos_id:= Position_ID AS Position_ID ,(SELECT `Position_Name` FROM tbl_position WHERE Position_ID = @pos_id) as Position FROM tbl_employees_information where Employees_ID = '$id') tbl_employees_information
	JOIN (SELECT Employees_ID, First_Name, Last_Name from tbl_personal_information where Employees_ID = '$id') tbl_personal_information ON tbl_employees_information.Employees_ID = tbl_personal_information.Employees_ID 
		LEFT JOIN (SELECT Employees_ID , Leave_Types , Leave_Date, Leave_Status from tbl_file_leave Where Leave_Date Between '$first_date' AND '$Last_date' AND Employees_ID = '$id') tbl_file_leave ON tbl_personal_information.Employees_ID = tbl_file_leave.Employees_ID
		LEFT JOIN (SELECT Employees_ID, Limit_Leave from tbl_leave_limit where Employees_ID = '$id') tbl_leave_limit ON tbl_file_leave.Employees_ID = tbl_leave_limit.Employees_ID WHERE tbl_file_leave.Leave_Status = 'Accepted'";
 	$query_leave_record1 = mysqli_query($conn,$sql1);
 	$rows = mysqli_num_rows($query_leave_record1);
 	$res_query_leave_record1=mysqli_fetch_assoc($query_leave_record1);
 	...
 	if(empty($res_query_leave_record1['Limit_Leave'])){
 		$leave_limit = 0;
 	}ELSE{
 		$leave_limit = $res_query_leave_record1['Limit_Leave'];
 	}
